...
@torch.no_grad()
def predict(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
        image=ROOT / 'data/images',  # file/dir/URL/glob, 0 for webcam
        data=ROOT / 'models/coco128.yaml',  # dataset.yaml path
    ...
    if webcam:
        ...
        dataset = LoadStreams(image, img_size=imgsz, stride=stride, auto=pt)
        bs = len(dataset)  # batch_size
    else:
        dataset = LoadImages(image, img_size=imgsz, stride=stride, auto=pt)
        bs = 1  # batch_size
